...
export default function Upload() {
  ...
  async function handleSubmit(event) {
    ...
    setProgress(4);
    try {
      const attendee = await addAttendee({ fullName: uploaderName.trim() });
      const uploaded = [];
      for (let index = 0; index < files.length; index += 1) {
        const file = files[index];
        const result = await uploadImageToCloudinary(file, (fileProgress) => {
    ...
    <PageTransition>
      <section className="container-shell">
        <div className="mx-auto max-w-4xl">
          <div className="mb-8">
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-accent">Contribute</p>
            <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">Upload photos</h1>
            <p className="mt-3 max-w-2xl text-gray-600 dark:text-gray-300">
              Add moments from your camera roll and share them with everyone at the event.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="soft-card overflow-hidden">
            <div className="grid gap-6 p-5 sm:p-8">
              <label className="block">
                ...
                <span className="focus-within:ring-accent flex min-h-12 items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 px-4 transition focus-within:ring-2 dark:border-white/10 dark:bg-white/5">
